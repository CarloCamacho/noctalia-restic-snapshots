...
| The diff header ran off the panel's right edge | reported from a real panel: two full 64-character snapshot ids in one unclamped label, so the line reached the edge and kept going | the header quotes restic's short form; the diff-path, file-listing and tag labels are bounded too, since `maxLines = 1` never stops a first line that is too wide |
| The retention preview survived the prune it acted on | reported as "once you hit Prune and confirm, it doesn't clear the preview list" — the list still named the removed snapshots and the button was still armed | a `forget` finishing re-runs the dry run, so the preview is re-stated against the repository as it now is (`keep 3, remove 7` → `keep 3, remove 0`), and the panel clears the stale list the moment you confirm |
| The *Prune now* confirmation could not be reached with a long removal list | reported from a real panel: 16 snapshots to remove, 12 rows visible, and no button — the note says "confirm below" and there was no way to reach below | the list scrolls inside `removal-scroll` and the button is appended after it, so no list length can push it off the bottom. `panel_render_test` asserts the rows are inside a scroll and the button is **not** |
